...
- The error lists the service under a different name (e.g. you asked for `tyfi-postgres`, available is `postgres`) — use the real name; do **not** allocate a duplicate.
...
**Service names are scoped to the project.** spout already knows which project you're in (from CWD). Use the bare service name (`postgres`), never project-prefixed (`tyfi-postgres`). If you don't know what the project has, run `spout env` once to enumerate everything.
