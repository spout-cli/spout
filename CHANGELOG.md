...
- `spout get <service>` and `spout rm <service>` now list the project's actual service names on failure instead of a bare "service not registered". When an agent guesses a wrong name (e.g. `tyfi-postgres` in a project whose service is registered as just `postgres`), the stderr message surfaces the real name plus a `spout env` pointer — so it self-corrects rather than allocating a duplicate. Exit code 1 unchanged. Empty-project failures suggest `spout alloc <service>`. New `SpoutError::ServiceNotRegisteredInProject` variant carries the context.
