...
- **RAII guard for terminal restoration.** `TerminalGuard`'s `Drop` impl runs `disable_raw_mode` and `LeaveAlternateScreen` even on panic. Verified by temporarily inserting `panic!()` inside the render loop — terminal state restored cleanly. This was the single biggest correctness risk for the TUI and it was cheap to get right because the pattern was already well-trodden in Ratatui's examples.
- **The TTY-gate is three independent conditions, all cheap.** `stdout().is_terminal()` + `!no_tui` + command-is-`ls` is readable at the call site (one `if` in `commands::ls`) and gives agents a zero-ambiguity rule: if you pipe or pass `--no-tui`, you get plain text. No heuristic, no magic.
...
- **Flaky test: `check_returns_true_for_free_port`.** Inherited from Stage 1 but hit on a re-run in this stage. The pattern is "bind an ephemeral port, drop the listener, assert the port is free" — inherently racy because the OS can hand the port to another process in the window between `drop` and the check. Left it alone for now (passes in isolation, rare in practice), but a future stage should either pin it to a held socket or use a port unlikely to be recycled.
...
- **Unit-testing TUI logic is tractable if you isolate the pure parts.** `collect_rows` and `is_exit_key` are both testable without a terminal, and they're where the interesting logic lives. The render loop itself is thin glue — the kind of code you smoke-test with `cargo run`, not with unit tests. Three of the six new TUI tests cover `collect_rows` edge cases (empty, filtered, multi-project separator).
